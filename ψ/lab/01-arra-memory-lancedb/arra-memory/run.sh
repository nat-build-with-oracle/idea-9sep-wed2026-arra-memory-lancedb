...
# Home Assistant's backups. The corpus lives there and nowhere else — one
# LanceDB directory holding every table.
export LANCEDB_URI="/data/lancedb"
...
bashio::log.info "  database: /data/lancedb"
...
if bashio::var.has_value "${OLLAMA_URL}"; then
    bashio::log.info "  search:   keyword (ngram FTS) + semantic via ${EMBEDDING_MODEL}"
else
    bashio::log.info "  search:   keyword only (ngram FTS) — set ollama_url for semantic"
fi
